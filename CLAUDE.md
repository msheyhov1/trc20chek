...
coverage, sanctioned}`). Центовое совпадение + порог ≥2 пары исключают ложняк на активных
трейдерах. Запускается в `_apply_flow` до WALLET-ветки. **Санкционная биржа** (HTX и т.п.): метку
депозитника ставим так же, но риск НЕ маскируем — `deposit_pattern.sanctioned=True` поднимает
адрес в `_compute_aml` (через `self_sanctioned_exch`) до `EntityType.SANCTIONED`, скор 100,
`DANGEROUS` («Депозитный кошелёк HTX … (санкционная биржа)»). Это закрывает кейс, когда sweep
шёл на санкционную биржу, а адрес ошибочно считался «личным кошельком».
...
# Прогнать тесты (должны быть все зелёные: 24/24)
pytest -v
