...
tests/test_core.py   # 16 тестов, моки на провайдеров через unittest.mock
```
...
# Прогнать тесты (должны быть все зелёные: 16/16)
pytest -v
...
- **Кеш опционален**: `check_address(addr, use_cache=False)` всегда дёргает свежие данные. Используется в эндпоинте `?no_cache=true`, в тестах и **в Telegram-боте** (бот всегда проверяет заново — для AML важна свежесть транзакций, кеш там не нужен). REST API и веб-форма кешируют по умолчанию
