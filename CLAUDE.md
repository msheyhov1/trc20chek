...
tests/test_core.py   # 13 тестов, моки на провайдеров через unittest.mock
```
...
3. Параллельно (`asyncio.gather`) дёргает TronScan + GoPlus + flow (переводы)
4. `_apply_tronscan` → `_apply_goplus` → `_apply_flow` → `_apply_local` (локальная БД имеет приоритет)
...
# Прогнать тесты (должны быть все зелёные: 13/13)
pytest -v
...
- **Кеш опционален**: `check_address(addr, use_cache=False)` всегда дёргает свежие данные. Используется в эндпоинте `?no_cache=true` и в тестах
