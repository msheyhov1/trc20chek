...
tests/test_core.py   # 11 тестов, моки на провайдеров через unittest.mock
```
...
3. Параллельно (`asyncio.gather`) дёргает TronScan + GoPlus
4. `_apply_tronscan` → `_apply_goplus` → `_apply_local` (локальная БД имеет приоритет)
...
# Прогнать тесты (должны быть все зелёные: 11/11)
pytest -v
