...
11. Предварительный вердикт (колбэк `on_preliminary`, если KYT будут запрошены)
12. Внешние KYT (Swapster + Bitok параллельно, если не туннель, с кешем на час)
13. **`_finalize_risk`**: `_compute_aml` (единственный расчёт) → `_apply_local` →
    `_apply_provider_gaps`
14. Журнал: запись + сравнение с прошлой ПОЛНОЙ проверкой
15. `_order_flags` — флаги по важности; кеш (если включён и проверка полная)

### Обрыв по общему дедлайну

Входные данные расчёта копятся в `_Progress` по мере получения, KYT пишут результат
сразу, как ответили. При обрыве `_finish_partial` досчитывает риск по собранному.
Раньше `_compute_aml` при обрыве не выполнялся вовсе, а прямое попадание в OFAC
определяется только в нём: санкционный адрес с медленным Bitok выходил «НЕТ
ДАННЫХ · 0/100». Бюджет Bitok теперь настенный (`asyncio.timeout`), как у Swapster.

### Предварительный вердикт

`check_address(..., on_preliminary=cb)` отдаёт в колбэк вердикт по on-chain данным
до запроса платных KYT. Считается на КОПИИ — у итога расчёт выполняется один раз.
Не отправляется, если KYT не будут запрошены (туннель, пустой адрес, KYT не
настроены). Бот показывает его правкой сообщения-прогресса.
...
`timeout`, `partial` (2-й хоп проверил не всех посредников), а для OFAC — `live` /
`cache` / `bundled` (откуда взят список).

**`AddressVerdict.is_degraded()`** — проверка неполная (`error`/`timeout`/`pending`).
Такой вердикт не кешируется, не сравнивается с прошлым в журнале и не считается
сменой уровня в мониторинге: иначе «TronScan не ответил» читается как «риск
снизился», а следующая полная проверка — как «вырос». `partial` сюда не входит
намеренно: хронический 429 по одному посреднику 2-го хопа иначе навсегда лишал бы
адрес мониторинга и повторял платную проверку каждый час.
...
- **2-й хоп** (`AML_HOP2`): топ-12 неизвестных посредников, вес `AML_HOP2_WEIGHT`.
  Сбой части посредников — статус `partial`, остальные результаты сохраняются
  (раньше один 429 выбрасывал всё, включая найденного «грязного»).
- **Собственные оценки TronScan**: серая метка — скор `GREY_TAG_SCORE`; доля объёма
  через рискованные переводы и контрагентов (`riskTransaction`,
  `normalAddressInfo.risk`) × `TRONSCAN_RISK_WEIGHT`. Для опознанной биржи — нет.
- **`aml.score_reason`** — «за что столько баллов» одной строкой; показывается под
  шкалой в боте и на сайте. Каждое место, поднимающее скор, обновляет и причину.
- **Жёсткие факты** (`_hard_fact`): санкционные списки OFAC/UK/EU и блокировка
  Tether. Ручная метка их НЕ отменяет — только мнения (KYT, красная метка, GoPlus).
  Депозит, чей якорь сам в OFAC, — санкционный, как бы якорь ни был помечен.
...
- **Пустой адрес** (`_is_untouched`): ни одной транзакции по счётчикам TronScan И ни
  одного перевода в разобранной истории. KYT считают граф транзакций — у такого
  адреса графа нет, и «0 % чисто» это не находка, а стоимость запроса. Условие
  требует согласия двух источников и исправных провайдеров: «источник не ответил»
  не должно превращаться в «адрес пустой».
...
- **Переклейка Swapster в биржу** (`_relabel_from_swapster`) — только для
  неразмеченных адресов без своих риск-сигналов, только в категорию LOW_RISK без
  слов HIGH/RISK/SANCTION/P2P/KYC, и без смены уровня. Раньше серая метка TronScan +
  «EXCHANGE 95 %» давали «биржа · безопасно · 0».
- **Кеш KYT** (`KYT_CACHE_SECONDS`, час): «Перепроверить» не оплачивает оба сервиса
  заново. Кешируется только готовый ответ; on-chain часть всегда свежая. В тестах
  кеш чистит autouse-фикстура в `conftest.py`.
...
проверены, поэтому подобраны узко; надёжный путь — разметить хот-кошелёк по адресу
(`/label` или `SERVICE_ADDRESSES`), дальше его депозитники подтянет funnel-эвристика.
Ключа «cryptopay» нет намеренно: Cryptopay (cryptopay.me) — отдельная компания.

**Теги-подделки** (`_IMPOSTOR_MARKERS`): «Fake Binance», «Binance Phishing»,
«Scam_Bybit» раньше матчились как сами биржи — «биржа · безопасно · 0» и отключённые
KYT. Теперь такой тег на самом адресе — SCAM, у контрагента — не биржа.

**Пыль не считается приходом** при распознавании депозитника: иначе пара пылинок от
адресов-двойников поднимала одноразовый свип до «уверенного» депозитника.
...
# Тесты (должны быть все зелёные: 411/411)
...
| `ADMIN_TG_IDS` | Кто ставит ручные метки. Пусто — любой из белого списка |
| `KYT_CACHE_SECONDS` | Сколько переиспользовать ответ Swapster/Bitok (3600, 0 — выкл.) |
| `TRUSTED_PROXY_HOPS` | Сколько прокси дописывают X-Forwarded-For (у Railway — 1) |
| `GREY_TAG_SCORE` / `TRONSCAN_RISK_WEIGHT` | Вес собственных оценок TronScan |
| `WATCHLIST_RETRY_SECONDS` | Повтор мониторинга после неполной проверки |
...
Команды: `/start`, `/help`, `/id`, `/status`, `/history` (свои; `/history all` —
всех), `/stats`, `/labels`, `/label`, `/unlabel` (при заданном `ADMIN_TG_IDS` —
только администраторы), `/watch`, `/unwatch`, `/watchlist`. Файлом — пакетная
проверка.

Проверка — в два шага: предварительный вердикт по данным блокчейна приходит правкой
сообщения-прогресса через секунды, итог с KYT — правкой того же сообщения.

Флаги в вердикте отсортированы по важности (`flag_rank`): бот показывает восемь, но
решающие (санкции, блокировка, «опасно») — всегда все. Раньше флаг внешнего AML
добавлялся последним и на «шумном» кошельке уходил за «…и ещё N».
...
- **Лимит на IP** берёт адрес, который дописал прокси (`TRUSTED_PROXY_HOPS`), а не
  начало X-Forwarded-For — его присылает клиент. Раньше лимит обходился подменой
- **Проверьте `storage` в `/health`**: контейнер работает от непривилегированного
  пользователя, а Railway, насколько известно, монтирует volume с правами root.
  Если там `unavailable` — не работают журнал, метки, мониторинг; решение —
  `RAILWAY_RUN_UID=0`
