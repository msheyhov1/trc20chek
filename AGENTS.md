...
**`AddressVerdict.is_degraded()`** — проверка неполная (`error`/`timeout`/`partial`/
`pending`). Такой вердикт не кешируется, не сравнивается с прошлым в журнале и не
считается сменой уровня в мониторинге: иначе «TronScan не ответил» читается как
«риск снизился», а следующая полная проверка — как «вырос».
...
# Тесты (должны быть все зелёные: 410/410)
